...
class CMLTmuxTests(BaseCMLTest):
    ...
    def test_cml_tmux_panes(self, mock_server):
        with self.get_context() as m:
            ...
            mock_window.split.return_value = mock_pane
            ...
            mock_server.return_value.new_session.assert_called_once_with(
                session_name="Mock Test-5f0d", kill_session=True
            )
            expected_calls = [
    ...
    def test_cml_tmux_panes24(self, mock_server):
        with self.get_context() as m:
            ...
            mock_server.assert_called_once()
            mock_server.return_value.new_session.assert_called_once_with(
                session_name="Mock Test 2_4-8811", kill_session=True
            )
            expected_calls = [
                call("printf '\\033]2;%s\\033\\\\' 'rtr-1'", suppress_history=True),
                call(
                    "ssh -t admin@localhost open /Mock Test 2.4/rtr-1/0",
